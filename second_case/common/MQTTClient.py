...
class OurMQTT:
    ...
    def myOnMessage(self, mqtt_client, userdata, msg):
        ...
        logger.info(f"MQTT client {self.clientID} received a message:\n {messageBody},\n on topic \n {msg.topic}.")
        #self.notifier.notify(False, msg.topic, msg.payload.decode("utf-8", "ignore"))
        logger.info(f"MQTT client {self.clientID} will be unsubscribe and disconnected.")
        self.stop()
    ...
    def stop(self):
        ...
        # Stopping the loop
        self.mqtt_client.loop_stop()
        # Finally, disconnecting the client from the broker
